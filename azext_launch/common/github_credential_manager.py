...
from azext_launch.common.utils import datetime_now_as_string, singleton
from azext_launch.common.const import LAUNCH_CLI_GITHUB_PAT_ENVKEY
...
class GithubCredentialManager():
    ...
    def _create_token(self, note=None):
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            logger.info("Storing PAT in environment variable")
        else:
    ...
    def get_token(self, note=None):
        import os
        github_pat = os.getenv(LAUNCH_CLI_GITHUB_PAT_ENVKEY, None)
        if github_pat:
            logger.warning('Using GitHub PAT token found in environment variable (%s).', LAUNCH_CLI_GITHUB_PAT_ENVKEY)
            return github_pat
        if not self.token:
